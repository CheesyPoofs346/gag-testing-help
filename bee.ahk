; Virage Grow a Garden Macro [BIZZY BEE UPDATE] revamp by X6 M and xTazerTx
...
honeyItems := ["Flower Seed Pack", "Nectarine Seed", "Hive Fruit Seed"
               , "Honey Sprinkler", "Bee Egg", "Bee Crate", "Honey Comb"
...
ShowWelcome:
    ...
    Gui, 99: Show, w788 h492 Center, Virage Grow a Garden Macro
return
...
Gui, Add, Tab, x10 y10 w500 h400 vMyTab -Wrap, Seeds|Gears|Eggs|Cosmetics|Honey|Settings|Donate|Credits|Help|
    ...
    Gui, Tab, 5
    Gui, Add, GroupBox, x23 y50 w475 h340 cFF800F, Honey Shop
    IniRead, SelectAllHoney, %settingsFile%, Honey, SelectAllHoney, 0
    Gui, Add, Checkbox, % "x50 y90 vSelectAllHoney gHandleSelectAll cFF800F " . (SelectAllHoney ? "Checked" : ""), Select All Honey
     Loop, % honeyItems.Length() {
        IniRead, eVal, %settingsFile%, Honey, Item%A_Index%, 0
        y := 125 + (A_Index - 1) * 25
        Gui, Add, Checkbox, % "x50 y" y " vHoneyItem" A_Index " gHandleSelectAll cWhite " . (eVal ? "Checked" : ""), % honeyItems[A_Index]
    }

    Gui, Add, CheckBox, % "x300 y90 vMakeHoney gHandleSelectAll cFF800F " . (MakeHoney ? "Checked" : ""), Make Honey
      Loop, % MakeHoney.Length() {
        IniRead, eVal, %settingsFile%, MakeHoney
        y := 125 + (A_Index - 1) * 25
        Gui, Add, Checkbox, % "x50 y" y " vMakeHoney " A_Index " gHandleSelectAll cWhite " . (eVal ? "Checked" : ""), % MakeHoney[A_Index]
    }
...
Gui, Add, Button, x400 y140 w85 h18 gSavePrivateURL Background202020, Save Link
    ...
    Gui, Font, s9 cD3D3D3, Segoe UI
    Gui, Add, Text, x50 y300, Forward key:
...
Gui, Add, Text,    x350 y360 w100 h24 +Right, 600
    ...
    Gui, Tab, 9
    Gui, Font, s9 cWhite Bold, Segoe UI
    Gui, Add, GroupBox, x23 y50 w475 h340 cD3D3D3, Help

    Gui, Font, s10 cWhite Bold, Segoe UI
    Gui, Add, Text, x35 y80 w300 h40, Please, make sure your roblox camera mode is set to the following : Default(classic)

  



    Gui, Show, w520 h425, Virage GAG Macro [BEE UPDATE] revamp by X6 M and xTazerTx
...
if(!targetPos = 1)
        ...
        else if(targetPos in 2, 3, 4)
            if(delta == 1 && (targetPos - 1) == 1 )
    ...
    if (honeyCompleted) {
        Sleep, 500
        uiUniversal("626262626262626066666606", 1, 1)
